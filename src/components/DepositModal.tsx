    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (signature) {
          ...
          const confirmResponse = await apiClient.post('/wallet/deposit/confirm', {
            signature: signature,
          });
          ...
          if (confirmResponse.data.success) {
            ...
            toast.error(`Deposit confirmation failed: ${confirmResponse.data.error || 'Unknown error'}`);
          }
        } else {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md bg-gray-900 border-gray-800">
        <DialogHeader>
          <DialogTitle className="text-white text-xl font-bold">Deposit SOL</DialogTitle>
        </DialogHeader>
